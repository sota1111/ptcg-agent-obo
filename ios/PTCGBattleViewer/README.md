...
相手側の手札は、ログにカード情報が含まれていても非公開のまま枚数だけを表示します。

トレーナーズの使用内容は `play-trainer` イベントの `player`、`cardName`、任意の `effect`
として記録します。タイムラインには使用者・カード名・効果を同じ時点のイベントとして表示し、
カード名や効果が空の場合も「名前不明」「説明なし」の安全な代替表示になります。

標準のiPhone 14縦向きでは、タイムライン、対戦相手、自分の盤面と自分の手札を縦スクロールせず
一画面に収めるコンパクト表示になります。手札にカード情報が含まれるログではカード名を表示し、
枚数しか含まれないログでもカード裏面と枚数を表示します。時点移動ボタンはアイコン表示ですが、VoiceOverでは
