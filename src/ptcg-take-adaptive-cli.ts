...
import {
  ...
  TAKE_EVALUATION_SCHEMA,
  evaluateTakeGame,
  validateTakeAdaptiveProfile,
  wilson95,
  type TakeAdaptiveProfile,
  type TakeEvaluationGame,
} from './lib/ptcgTakeAdaptive.js';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const output = path.resolve(root, process.argv[2] ?? 'artifacts/ptcg-take-adaptive/sot-1849');
const profileFile = path.join(root, 'config', 'ptcg_take_adaptive.json');
const profile = JSON.parse(fs.readFileSync(profileFile, 'utf8')) as TakeAdaptiveProfile;
const errors = validateTakeAdaptiveProfile(profile);
if (errors.length) throw new Error(errors.join('; '));

fs.mkdirSync(output, { recursive: true });
const checkpointFile = path.join(output, 'checkpoint.json');
let games: TakeEvaluationGame[] = [];
...
for (const opponent of ['baseline', ...TAKE_CROSS_PLAY_OPPONENTS] as const) {
  const variants =
    ...
    for (let offset = 0; offset < profile.exploration.seedCount; offset++) {
      for (const seat of ['first', 'second'] as const) {
        ...
        if (games.length % profile.exploration.checkpointEvery === 0)
          fs.writeFileSync(
            checkpointFile,
            `${JSON.stringify({ schemaVersion: TAKE_EVALUATION_SCHEMA, profileId: profile.archetype.id, games }, null, 2)}\n`
          );
      }
    }
}
...
const results = [
  ...
  ...TAKE_CROSS_PLAY_OPPONENTS.map((opponent) => summarize('candidate', opponent)),
];
const report = {
  schemaVersion: TAKE_EVALUATION_SCHEMA,
...
console.log(`take evaluation completed: ${games.length} games`);
