...
import {
  ...
  MATSU_EVALUATION_SCHEMA,
  evaluateMatsuGame,
  validateMatsuStabilityProfile,
  wilson95,
  type MatsuEvaluationGame,
  type MatsuStabilityProfile,
} from './lib/ptcgMatsuStability.js';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const output = path.resolve(root, process.argv[2] ?? 'artifacts/ptcg-matsu-stability/sot-1848');
const profileFile = path.join(root, 'config', 'ptcg_matsu_stability.json');
const profile = JSON.parse(fs.readFileSync(profileFile, 'utf8')) as MatsuStabilityProfile;
const errors = validateMatsuStabilityProfile(profile);
if (errors.length) throw new Error(errors.join('; '));

fs.mkdirSync(output, { recursive: true });
const checkpointFile = path.join(output, 'checkpoint.json');
let games: MatsuEvaluationGame[] = [];
...
const opponents = ['baseline', ...MATSU_CROSS_PLAY_OPPONENTS] as const;
    ...
    for (let offset = 0; offset < profile.exploration.seedCount; offset++) {
      for (const seat of ['first', 'second'] as const) {
        ...
        if (games.length % profile.exploration.checkpointEvery === 0)
          fs.writeFileSync(
            checkpointFile,
            `${JSON.stringify({ schemaVersion: MATSU_EVALUATION_SCHEMA, profileId: profile.archetype.id, games }, null, 2)}\n`
          );
      }
    }
  }
...
const results = [
  ...
  ...MATSU_CROSS_PLAY_OPPONENTS.map((opponent) => summarize('candidate', opponent)),
];
const report = {
  schemaVersion: MATSU_EVALUATION_SCHEMA,
...
console.log(`matsu evaluation completed: ${games.length} games`);
